...
class BotConfig(BaseSettings):
    ...
    db_path: str = "eco_memory.db"  # ✅ Относительный путь от скрипта
    ...
    try:
        ...
        async with aiosqlite.connect(db_path) as db:
            ...
            """) as cursor:
                ...
                text += "=" * 40 + "\n"
                text += "🔗 Полная лента: " + config.signature_economy
        
        # Отправляем в канал
        channel_id = config.tg_channel_economy
    ...
    if config.autopost_enabled:
        try:
            ...
            digest_task = asyncio.create_task(
                schedule_digest_for_category(llm, tg_client, config, "economy", hour=21)
            )
