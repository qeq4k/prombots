...
get_metric() {
    local port=$1
    local metric=$2
    curl -s "http://localhost:$port/metrics" 2>/dev/null | grep "^$metric" | awk '{print $2}'
}
...
show_bot_metrics() {
    ...
    # Суммируем по всем label (если есть)
    posts_sum=$(curl -s "http://localhost:$port/metrics" 2>/dev/null | grep "^bot_posts_sent_total" | awk -F'{' '{print $1}' | sort -u | xargs -I {} curl -s "http://localhost:$port/metrics" 2>/dev/null | grep "^{}" | awk '{sum+=$2} END {print sum}')
